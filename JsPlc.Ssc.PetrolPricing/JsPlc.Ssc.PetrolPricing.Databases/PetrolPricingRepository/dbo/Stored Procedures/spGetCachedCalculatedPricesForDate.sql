...
----DEBUG:START
--DECLARE @forDate DATE = '2017-08-31'
...
----DEBUG:END
	...
	--
	-- Test for Historical data - NEVER Cache
	--
	IF CONVERT(DATE, @ForDate) <> CONVERT(DATE, GETDATE())
	BEGIN
		--
		-- Remove and PriceSnapshots before Today
		--
		DECLARE @SnapshotsToRemove TABLE (PriceSnapshotId INT)
		INSERT INTO @SnapshotsToRemove (PriceSnapshotId)
		SELECT
			ps.PriceSnapshotId
		FROM
			dbo.PriceSnapshot ps
		WHERE
			ps.DateTo < CONVERT(DATE, GetDate());

		DELETE FROM dbo.PriceSnapshot WHERE PriceSnapshotId IN (SELECT PriceSnapshotId FROM @SnapshotsToRemove)
		DELETE FROM dbo.PriceSnapshotRow WHERE PriceSnapshotId IN (SELECT PriceSnapshotId FROM @SnapshotsToRemove)
