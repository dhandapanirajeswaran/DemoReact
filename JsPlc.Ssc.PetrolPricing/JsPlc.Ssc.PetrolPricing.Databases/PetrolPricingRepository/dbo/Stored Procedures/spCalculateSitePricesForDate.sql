...
----DEBUG:START
--DECLARE @forDate DATE = '2017-06-02'
...
-- Limit backwards search to X days
DECLARE @SearchBackwardsInDays INT  = 7
DECLARE @MinSearchBackwardsDate DATE = DATEADD(Day, -@SearchBackwardsInDays, @StartOfToday)
...
CalculatedTemp AS (
	...
	WHERE
		...
		AND
		sp.DateOfCalc >= @MinSearchBackwardsDate -- limit to @SearchBackwardsInDays
		AND 
...
SELECT
	...
	CONVERT(INT, COALESCE(stc.DriveTime/5,0)) [DriveTimePence]
FROM 
	AllFuelPrices afp
