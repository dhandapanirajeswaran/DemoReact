...
TomorrowPrices AS (
	...
	WHERE
		sp.Id = (SELECT TOP 1 Id FROM dbo.SitePrice WHERE SiteId = st.SiteId AND FuelTypeId = st.FuelTypeId AND SuggestedPrice > 0 AND DateOfCalc <= @StartOfToday ORDER BY DateOfCalc DESC)
),
Calculated AS (
	SELECT 
		st.SiteId [SiteId],
