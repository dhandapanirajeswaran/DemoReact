...
----DEBUG:START
--DECLARE @forDate DATE = '2017-08-17'
...
DECLARE @PriceReasonFlags_ManualOverride INT = 0x00004000
	...
	--
	-- Check if the Daily Catalist file is present
	--
	DECLARE @DailyCatalistFileExists BIT = 0
	IF EXISTS(SELECT TOP 1 NULL FROM dbo.FileUpload WHERE StatusId = 10 AND UploadTypeId = 1 AND CONVERT(DATE, UploadDateTime) = @forDate)
		SET @DailyCatalistFileExists = 1
	--
	...
	WHERE
		...
		AND
		CONVERT(DATE, UploadDateTime) = @forDate

	DECLARE @Today_DateOfCalc DATE

	SET @Tomorrow_DateOfCalc = CONVERT(DATE, @ForDate)

	--
	-- Find 'Today' Catalist file upload Date (search backwards to handle weekend/Bank Holidays)
	--
	SELECT TOP 1
		@Today_DateOfCalc = CONVERT(DATE, fu.UploadDateTime)
	FROM
		dbo.FileUpload fu
	WHERE
		...
		AND
		fu.UploadDateTime <= @Tomorrow_DateOfCalc
	ORDER BY
	...
	TodayAndTomorrowPrices AS (
		SELECT
			...
			COALESCE(tomorrow.Markup, 0) [Markup],
			CASE
			...
			CASE 
				WHEN today.OverriddenPrice > 0 THEN @PriceReasonFlags_ManualOverride
				ELSE COALESCE(today.PriceReasonFlags, @PriceReasonFlags_NoSuggestedPrice)
			END [TodayPriceReasonFlags],
			COALESCE(tomorrow.PriceReasonFlags, 0) [TomorrowPriceReasonFlags],
			COALESCE(tomorrow.DriveTimeMarkup, 0) [DriveTimeMarkup],
		...
		FROM 
			...
			LEFT JOIN dbo.SitePrice tomorrow ON tomorrow.SiteId = sfc.SiteId AND tomorrow.FuelTypeId = sfc.FuelTypeId AND tomorrow.DateOfCalc = @Tomorrow_DateOfCalc AND @DailyCatalistFileExists = 1
			LEFT JOIN dbo.SiteToCompetitor stc ON tomorrow.CompetitorId = stc.CompetitorId AND stc.SiteId = sfc.SiteId
	)
	SELECT
		tatp.*
	FROM 
		TodayAndTomorrowPrices tatp
...
----DEBUG
--SELECT
--	@Today_DateOfCalc [@Today_DateOfCalc],
--	@Tomorrow_DateOfCalc [@Tomorrow_DateOfCalc]
